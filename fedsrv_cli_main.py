...
def fedsrv_cli():
    ...
    kg_context = ""  # Empty at startup, no placeholder
    if verbose_mode >= 1:
        system_tokens = estimate_tokens([{"role": "system", "content": grok_config.get("system-prompts", [{}])[0].get("content", "")}])
        history_tokens = estimate_tokens(memory)
        kg_tokens = estimate_tokens([{"content": kg_context}]) if kg_context else 0  # 0 tokens if empty
        total_tokens = system_tokens + history_tokens + kg_tokens
        click.echo(f"{Fore.YELLOW}Token breakdown on startup:{Style.RESET_ALL}")
        ...
        click.echo(f"{Fore.YELLOW}  Total tokens: {total_tokens}/{token_limit}{Style.RESET_ALL}")
    if verbose_mode >= 2:
        system_prompt_content = grok_config.get("system-prompts", [{}])[0].get("content", "") or "<No System Prompt>"
        kg_context_content = kg_context or "<No KG Matches>"
        memory_content = memory or ["<No Conversation Memory>"]
        click.echo(f"{Fore.YELLOW}Exact token content on startup:{Style.RESET_ALL}")
    ...
    while True:
        ...
        if command == "/help":
            ...
            click.echo("- /mode:verbose 0/1/2: Sets verbosity (0=standard, 1=verbose, 2=extreme, currently {}).".format(verbose_mode))
            ...
            system_tokens = estimate_tokens([{"role": "system", "content": combined_system_prompt}])
            history_tokens = estimate_tokens(memory)
            kg_tokens = estimate_tokens([{"content": kg_context}]) if kg_context else 0  # 0 tokens if empty
            total_tokens = system_tokens + history_tokens + kg_tokens
            click.echo(f"{Fore.YELLOW}Current token breakdown:{Style.RESET_ALL}")
            click.echo(f"{Fore.YELLOW}  System tokens: {system_tokens}{Style.RESET_ALL}")
            click.echo(f"{Fore.YELLOW}  History tokens: {history_tokens}{Style.RESET_ALL}")
            click.echo(f"{Fore.YELLOW}  KG context tokens: {kg_tokens}{Style.RESET_ALL}")
            click.echo(f"{Fore.YELLOW}  Total tokens: {total_tokens}/{token_limit}{Style.RESET_ALL}")
            if verbose_mode >= 2:
                system_prompt_content = combined_system_prompt or "<No System Prompt>"
                kg_context_content = kg_context or "<No KG Matches>"
                memory_content = memory or ["<No Conversation Memory>"]
                click.echo(f"{Fore.YELLOW}Exact token content:{Style.RESET_ALL}")
                click.echo(f"{Fore.YELLOW}  System prompt: {system_prompt_content}{Style.RESET_ALL}")
                click.echo(f"{Fore.YELLOW}  KG context: {kg_context_content}{Style.RESET_ALL}")
                click.echo(f"{Fore.YELLOW}  Conversation Memory:{Style.RESET_ALL}")
                if memory_content == ["<No Conversation Memory>"]:
            ...
            for item in jsonld_graph.get('@graph', []):
                if item.get('@type') == 'Class' and '@id' in item:
                    kg_labels.append(item['@id'])
            kg_context = ""  # Reset to empty after reload
            if verbose_mode >= 1:
                click.echo(f"{Style.BRIGHT}Knowledge Graph reloaded with {len(kg_labels)} class labels.{Style.RESET_ALL}")
        elif command == "/mcp":
            if verbose_mode >= 1:
                ...
                while True:
                    ...
                    if prompt == "/help":
                        ...
                        try:
                            ...
                            kg_context = ", ".join(str(m) for m in kg_matches) if kg_matches else ""
                            if not isinstance(kg_context, str):
                                if verbose_mode >= 1:
                                    click.echo(f"{Fore.RED}Invalid KG context type: {type(kg_context)}{Style.RESET_ALL}")
                                kg_context = ""
                            if verbose_mode >= 2:
                                click.echo(f"{Fore.YELLOW}KG context: {kg_context or '<No KG Matches>'}{Style.RESET_ALL}")
                            ...
                            # Validate messages
                            messages = [
                                {"role": "system", "content": f"{combined_system_prompt}\nKG context: {kg_context}" if kg_context else combined_system_prompt},
                            ...
                            # Log token breakdown before Big LLM request
                            if verbose_mode >= 1:
                                system_tokens = estimate_tokens([{"role": "system", "content": combined_system_prompt}])
                                kg_tokens = estimate_tokens([{"content": kg_context}]) if kg_context else 0
                                history_tokens = estimate_tokens(last_three + history_matches)
                                user_tokens = estimate_tokens([{"role": "user", "content": prompt}])
                                total_tokens = system_tokens + kg_tokens + history_tokens + user_tokens
                                click.echo(f"{Fore.YELLOW}Token breakdown for Big LLM request:{Style.RESET_ALL}")
                                click.echo(f"{Fore.YELLOW}  System tokens: {system_tokens}{Style.RESET_ALL}")
                                click.echo(f"{Fore.YELLOW}  History tokens: {history_tokens}{Style.RESET_ALL}")
                                click.echo(f"{Fore.YELLOW}  KG context tokens: {kg_tokens}{Style.RESET_ALL}")
                                click.echo(f"{Fore.YELLOW}  Total tokens: {total_tokens}/{token_limit}{Style.RESET_ALL}")
                            if verbose_mode >= 2:
                                system_prompt_content = combined_system_prompt or "<No System Prompt>"
                                kg_context_content = kg_context or "<No KG Matches>"
                                memory_content = last_three + history_matches or ["<No Conversation Memory>"]
                                click.echo(f"{Fore.YELLOW}Exact token content for Big LLM request:{Style.RESET_ALL}")
                                click.echo(f"{Fore.YELLOW}  System prompt: {system_prompt_content}{Style.RESET_ALL}")
                                click.echo(f"{Fore.YELLOW}  KG context: {kg_context_content}{Style.RESET_ALL}")
                                click.echo(f"{Fore.YELLOW}  Conversation Memory:{Style.RESET_ALL}")
                                if memory_content == ["<No Conversation Memory>"]:
                                    ...
                                    for msg in memory_content:
                                        click.echo(f"{Fore.YELLOW}    {msg['role'].capitalize()}: {msg['content'][:50]}...{Style.RESET_ALL}")
                                click.echo(f"{Fore.YELLOW}  User prompt: {prompt}{Style.RESET_ALL}")
                            ...
                            if grok_ok:
                                ...
                                if verbose_mode >= 1:
                                    ...
                                    kg_tokens = estimate_tokens([{"content": kg_context}]) if kg_context else 0
                                    history_tokens = estimate_tokens(memory)
                                    total_tokens = system_tokens + kg_tokens + history_tokens
                                    click.echo(f"{Fore.YELLOW}Token breakdown after Grok response:{Style.RESET_ALL}")
                                    click.echo(f"{Fore.YELLOW}  System tokens: {system_tokens}{Style.RESET_ALL}")
                                    click.echo(f"{Fore.YELLOW}  History tokens: {history_tokens}{Style.RESET_ALL}")
                                    click.echo(f"{Fore.YELLOW}  KG context tokens: {kg_tokens}{Style.RESET_ALL}")
                                    click.echo(f"{Fore.YELLOW}  Total tokens: {total_tokens}/{token_limit}{Style.RESET_ALL}")
                                if verbose_mode >= 2:
                                    ...
                                    kg_context_content = kg_context or "<No KG Matches>"
                                    memory_content = memory or ["<No Conversation Memory>"]
                                    click.echo(f"{Fore.YELLOW}Exact token content after Grok response:{Style.RESET_ALL}")
                                    click.echo(f"{Fore.YELLOW}  System prompt: {system_prompt_content}{Style.RESET_ALL}")
                                    click.echo(f"{Fore.YELLOW}  KG context: {kg_context_content}{Style.RESET_ALL}")
                                    click.echo(f"{Fore.YELLOW}  Conversation Memory:{Style.RESET_ALL}")
                                    if memory_content == ["<No Conversation Memory>"]:
                            ...
                            if mcp_ok:
                                ...
                                if verbose_mode >= 1:
                                    ...
                                    history_tokens = estimate_tokens(memory)
                                    total_tokens = system_tokens + kg_tokens + history_tokens
                                    click.echo(f"{Fore.YELLOW}Token breakdown after MCP response:{Style.RESET_ALL}")
                                    click.echo(f"{Fore.YELLOW}  System tokens: {system_tokens}{Style.RESET_ALL}")
                                    click.echo(f"{Fore.YELLOW}  History tokens: {history_tokens}{Style.RESET_ALL}")
                                    click.echo(f"{Fore.YELLOW}  KG context tokens: {kg_tokens}{Style.RESET_ALL}")
                                    click.echo(f"{Fore.YELLOW}  Total tokens: {total_tokens}/{token_limit}{Style.RESET_ALL}")
                                if verbose_mode >= 2:
                                    ...
                                    kg_context_content = kg_context or "<No KG Matches>"
                                    memory_content = memory or ["<No Conversation Memory>"]
                                    click.echo(f"{Fore.YELLOW}Exact token content after MCP response:{Style.RESET_ALL}")
                                    click.echo(f"{Fore.YELLOW}  System prompt: {system_prompt_content}{Style.RESET_ALL}")
                                    click.echo(f"{Fore.YELLOW}  KG context: {kg_context_content}{Style.RESET_ALL}")
                                    click.echo(f"{Fore.YELLOW}  Conversation Memory:{Style.RESET_ALL}")
                                    if memory_content == ["<No Conversation Memory>"]:
                        ...
                        if verbose_mode >= 1:
                            click.echo(f"{Fore.YELLOW}Input ignored InMemory MCP mode. Enter /mode:big-llm to send requests.{Style.RESET_ALL}")
        elif command == "/back":
            if verbose_mode >= 1:
